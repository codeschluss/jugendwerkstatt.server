...
public class QuestionService extends DataService<QuestionEntity, QuestionPredicateBuilder> {
  ...
  public Optional<QuestionEntity> getExisting(QuestionEntity entity) {
    return entity.getItem() == null || entity.getItem().isEmpty()
        || entity.getQuestionnaire() == null ? Optional.empty()
  ...
  public Optional<QuestionEntity> getByItemAndQuestionnaire(String item, String questionnaireId) {
    return repo.findOne(predicate.withItem(item).and(predicate.withQuestionnaire(questionnaireId)));
  }

  public double calculateAverageRating(QuestionEntity question, Integer year) {
    var answers =
        answerService.readAll(answerService.query().and(answerService.getPredicate().withYear(year))
    ...
    return answers != null && answers.size() > 0 ? sum / answers.size() : sum;
  }
}
