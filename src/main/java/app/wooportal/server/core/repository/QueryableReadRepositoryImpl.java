...
public class QueryableReadRepositoryImpl<T extends BaseEntity> extends QuerydslJpaPredicateExecutor<T>
  ...
  @Override
  public PageableList<T> findAll(Query<T> query) {
    return query.getPage() != null
        ? new PageableList<>(findAll(query.getPredicate(), query.getPage(), query.getGraph()))
        : new PageableList<>(findAll(query.getPredicate(), query.getSort(), query.getLimit(), query.getGraph()));
  }
